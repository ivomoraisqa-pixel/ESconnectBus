window.Components = {
  kpiCard({ icon, iconBg, iconColor, title, value, subtitle, trend, trendValue, borderColor, extra = '', tag = '', tagColor = '', variant = '' }) {
    let trendHtml = '';
    if (trend) {
      const trendClass = trend === 'up' ? 'positive' : 'negative';
      trendHtml = `<span class="kpi-trend ${trendClass}">${trendValue}</span>`;
    }
    const tagHtml = tag ? `<span class="badge" style="background-color:${tagColor}20; color:${tagColor}; margin-left:auto;">${tag}</span>` : '';
    
    // If a variant is not passed but a known color is passed, map it.
    let variantClass = variant ? `variant-${variant}` : '';
    if (!variantClass && borderColor) {
       if (borderColor.includes('3B82F6')) variantClass = 'variant-blue';
       if (borderColor.includes('F59E0B')) variantClass = 'variant-orange';
       if (borderColor.includes('EF4444') || borderColor.includes('EC4899')) variantClass = 'variant-red';
    }

    return `
      <div class="kpi-card ${variantClass}" style="position:relative;">
        <div class="kpi-icon" ${!variantClass && iconBg ? `style="background-color:${iconBg}; color:${iconColor};"` : ''}>
          ${icon}
        </div>
        <div class="kpi-info">
  ...
  badge(status) {
    ...
    let badgeClass = 'badge-encerrada'; // default
    if (s === 'online' || s === 'ativa' || s === 'ativo' || s === 'sucesso') badgeClass = 'badge-online';
    else if (s === 'offline' || s === 'inativo' || s === 'erro') badgeClass = 'badge-offline';
    else if (s === 'instalação' || s === 'instalacao' || s === 'pausada' || s === 'aviso') badgeClass = 'badge-instalacao';
    else if (s === 'agendada') badgeClass = 'badge-agendada';
    
    return `<span class="badge ${badgeClass}">${status}</span>`;
  },
  
  updateItem({ icon, iconBg, title, subtitle, badge, badgeColor }) {
    return `
      <div class="update-item">
        <div class="update-icon" style="background-color:${iconBg}; color:${badgeColor};">
          ${icon}
        </div>
        <div class="update-info">
          <div class="update-title">${title}</div>
          <div class="update-date">${subtitle}</div>
        </div>
        ${badge ? `<div class="update-badge" style="color:${badgeColor}; background-color:${badgeColor}15;">${badge}</div>` : ''}
      </div>
    `;
  },
  ...
  searchBar(placeholder, id) {
    return `
      <div class="search-input-wrapper" style="width:250px;">
        <span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#9CA3AF;pointer-events:none;">${this.icon('search', 16)}</span>
        <input id="${id}" class="form-input" type="text" placeholder="${placeholder}" style="padding-left:32px;" />
      </div>
    `;
  },
  
  filterSelect(id, options, placeholder) {
    const opts = options.map(o => `<option value="${o}">${o}</option>`).join('');
    return `
      <select id="${id}" class="form-select" style="cursor:pointer; width:auto;">
        ${opts}
      </select>
    `;
  },
  ...
  actionButtons(actions) {
    const btns = actions.map(a => `
      <button class="btn-icon" title="${a.title}" ${a.onclick ? `onclick="${a.onclick}"` : ''}>
        ${this.icon(a.icon, 16)}
      </button>
    `).join('');
    return `<div class="actions-group">${btns}</div>`;
  },
      ...
      <div id="${id}" class="modal-overlay" style="display:none;">
        <div class="modal">
          <div class="modal-header">
            ...
            <button onclick="Components.hideModal('${id}')" class="btn-icon">${this.icon('x', 20)}</button>
          </div>
  ...
  icon(name, size = 20) {
    const icons = {
      'monitor': '<path d="M2 5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V5zm10 14h-4m2 0v3"/>',
      'bar-chart': '<path d="M12 20V10m-6 10V4m12 16v-8"/>',
      'bus': '<path d="M4 16v3a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-3m12 0v3a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1v-3M4 16h16V7a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v9zm4-4h8"/>',
      ...
      'edit': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>`,
      ...
      'chart': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg>`,
      ...
      'plus': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="5" y2="19"/><line x1="5" x2="19" y1="12" y2="12"/></svg>`,
      ...
      'megaphone': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 11 18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg>`,
      ...
      'plug': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22v-5"/><path d="M9 8V2"/><path d="M15 8V2"/><path d="M18 8v5a4 4 0 0 1-4 4h-4a4 4 0 0 1-4-4V8Z"/></svg>`,
      ...
      'info': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="16" y2="12"/><line x1="12" x2="12.01" y1="8" y2="8"/></svg>`,
      'wifi': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><line x1="12" x2="12.01" y1="20" y2="20"/></svg>`,
      'arrow-up': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="19" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>`,
      'arrow-down': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="5" y2="19"/><polyline points="19 12 12 19 5 12"/></svg>`,
      'power': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18.36 6.64a9 9 0 1 1-12.73 0"/><line x1="12" x2="12" y1="2" y2="12"/></svg>`,
      'refresh-cw': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>`,
      ...
      'hard-drive': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" x2="2" y1="12" y2="12"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/><line x1="6" x2="6.01" y1="16" y2="16"/><line x1="10" x2="10.01" y1="16" y2="16"/></svg>`,
      'tool': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>`,
      'camera': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/></svg>`,
      'thermometer': `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 4v10.54a4 4 0 1 1-4 0V4a2 2 0 0 1 4 0Z"/></svg>`,
